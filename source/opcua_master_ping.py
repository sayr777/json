...
if __name__ == "__main__":
     ...
     try:
         ...
         server.set_server_name("Server_ping")
         uri = "http://server_ping"
         ...
         for i in range(0,len(dt)):
             ...
             opcObject.append(i)
             opcObjectVar.append(i)
             opcObject[i] = objects.add_object(idx,  namePoint[i])
             opcObjectVar[i]=opcObject[i].add_variable(idx, "Connection",0)
             i+=1
         ...
         while True:
             for i in range(0,units):
                     try:
                         ...
                         if(pingData == 1):
                             opcObjectVar[i].set_value(1)
                         else:
                             opcObjectVar[i].set_value(0)

                         print ( 'master_ping '+time.strftime('%d-%m-%Y %H:%M:%S'), host[i] , pingData,mes, namePoint[i])
